...
function ComparisonPreviewSection({ categoryName, tableColumns }) {
  const previewColumns = Array.isArray(tableColumns)
    ? tableColumns.map((column) => String(column || "").trim()).filter(Boolean)
    : [];
  const previewProducts = [
    `${categoryName || "Sample"} A`,
    `${categoryName || "Sample"} B`,
    `${categoryName || "Sample"} C`,
  ];
    ...
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h3 className="mt-2 text-2xl font-semibold text-slate-900">Public filter layout</h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            This preview shows how the filter comparison panel will line up product names against
            the category table columns.
          </p>
        </div>
        <button
          type="button"
          className="inline-flex items-center rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-400"
          disabled
        >
          Difference
        </button>
      ...
      {previewColumns.length ? (
          ...
          <table className="min-w-[640px] w-full border-collapse text-sm">
            <thead>
              <tr className="bg-slate-50">
                ...
                {previewProducts.map((label) => (
                  <th
                    key={label}
                    className="border-b border-slate-200 px-4 py-4 text-left text-xs font-bold uppercase tracking-[0.2em] text-slate-500"
                  >
                    <div className="space-y-1">
                      <p className="text-sm font-semibold uppercase tracking-normal text-slate-900">
                        {label}
                      </p>
            ...
            <tbody>
              {previewColumns.map((column, index) => (
                <tr key={column} className={index % 2 === 1 ? "bg-slate-50/60" : ""}>
                  <th className="border-b border-slate-200 px-4 py-3 text-left text-sm font-semibold text-slate-900">
                    {column}
                  </th>
                  {previewProducts.map((label, productIndex) => (
                    <td
                      key={`${label}-${column}`}
                      className="border-b border-slate-200 px-4 py-3 text-slate-600"
                    >
                      {buildPreviewCellValue(column, productIndex, previewColumns.length)}
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <div className="mt-5 rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-500">
          Add table columns to see the comparison preview.
        </div>
      )}
    </section>
  );
...
function buildPreviewCellValue(column, productIndex, totalColumns) {
  ...
  return totalColumns > 2 && productIndex === 2 ? `${base} variant` : `${base} value`;
}
